(() => {
  ...
  let busy=false,summaryAt=0,live=null,generation=0,wasActive=false;
  ...
  function clear(){generation++;summaryAt=0;live=null;if(dialog.open)dialog.close();frame.removeAttribute('srcdoc');for(const id of ['Live','Sessions','Clicks','Leads'])text('adminAnalytics'+id,'—');}
  ...
  byId('openAdminAnalytics').addEventListener('click',async()=>{
    if(!active())return;dialog.showModal();text('adminAnalyticsReportStatus','Loading report…');byId('adminAnalyticsReportStatus').hidden=false;frame.hidden=true;const ticket=generation,userId=currentUser.id;
    try{const {html}=await fetchApiWithAuth('/api/admin/analytics/report',{timeoutMs:20000});if(ticket!==generation||!active()||currentUser.id!==userId||!dialog.open)return;
      const doc=new DOMParser().parseFromString(html,'text/html');doc.querySelectorAll('script,style').forEach(el=>el.remove());
      const css=doc.createElement('link');css.rel='stylesheet';css.href=new URL('/admin-analytics-report.css',location.origin).href;doc.head.append(css);
      frame.srcdoc='<!doctype html>'+doc.documentElement.outerHTML;
    }catch{if(ticket===generation)text('adminAnalyticsReportStatus','Could not load analytics. Close this window and try again.');}
  });
  frame.addEventListener('load',()=>{if(!dialog.open)return;theme();renderLive();frame.hidden=false;byId('adminAnalyticsReportStatus').hidden=true;});
  byId('closeAdminAnalytics').addEventListener('click',()=>dialog.close());
  dialog.addEventListener('close',()=>{frame.removeAttribute('srcdoc');byId('openAdminAnalytics').focus();});
