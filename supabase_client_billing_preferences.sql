...
create policy if not exists "client_billing_select_own"
  on public.client_billing_preferences
  for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "client_billing_insert_own"
  on public.client_billing_preferences
  for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "client_billing_update_own"
  on public.client_billing_preferences
  for update
  to authenticated
  using (auth.uid() = user_id)
