...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      if (pathname === "/api/admin/privacy-maintenance/run" && request.method === "POST") {
      ...
      if (pathname === "/api/admin/invoices/send-topup-test" && request.method === "POST") {
        return handleAdminTopupInvoiceSendTest(request, env);
      }
      ...
      if (pathname === "/api/admin/billing/wise/receipts" && request.method === "GET") {
        return handleAdminWiseReceiptsList(request, env, url);
      }
      if (pathname === "/api/admin/billing/wise/sync" && request.method === "POST") {
        return handleAdminWiseSync(request, env);
      }
      if (pathname === "/api/admin/billing/wise/receipts/resolve" && request.method === "POST") {
        return handleAdminWiseReceiptResolve(request, env);
      }
      ...
      if (pathname === "/api/privacy/delete" && request.method === "POST") {
        return handlePrivacyDelete(request, env);
      }
      if (pathname === "/api/wise/webhooks" && request.method === "POST") {
        return handleWiseWebhook(request, env, ctx);
      }
      ...
      if (pathname === "/api/shopify/public-config" && request.method === "GET") {
        return handleShopifyPublicConfig(request, env);
      }
      if (pathname === "/api/shopify/embedded/session" && request.method === "GET") {
        return handleShopifyEmbeddedSession(request, env, url);
      }
      if (pathname === "/api/shopify/webhooks/compliance" && request.method === "POST") {
        return handleShopifyComplianceWebhook(request, env);
      }
      ...
      if (pathname === "/api/wix/locations" && request.method === "GET") {
        return handleWixLocations(request, env, url);
      }
      if (
        (pathname === "/api/wix/settings" || pathname === "/api/wix/setting") &&
        request.method === "GET"
      ) {
        return handleWixSettingsGet(request, env, url);
      }
      if (
        (pathname === "/api/wix/settings" || pathname === "/api/wix/setting") &&
        request.method === "POST"
      ) {
        return handleWixSettingsPost(request, env);
      }
      if (pathname === "/api/wix/disconnect" && request.method === "POST") {
        return handleWixDisconnect(request, env);
      }
      ...
      if (pathname === "/api/woocommerce/disconnect" && request.method === "POST") {
        return handleWooCommerceDisconnect(request, env);
      }
      if (pathname === "/api/linkedin/install-link" && request.method === "POST") {
        return handleLinkedInInstallLink(request, env);
      }
      if (pathname === "/api/linkedin/callback" && request.method === "GET") {
        return handleLinkedInCallback(request, env, url);
      }
      if (pathname === "/api/linkedin/connection" && request.method === "GET") {
        return handleLinkedInConnection(request, env, url);
      }
      if (pathname === "/api/linkedin/select-organization" && request.method === "POST") {
        return handleLinkedInSelectOrganization(request, env);
      }
      if (pathname === "/api/linkedin/post" && request.method === "POST") {
        return handleLinkedInPost(request, env);
      }
      if (pathname === "/api/linkedin/disconnect" && request.method === "POST") {
      ...
      if (pathname === "/api/shopify/disconnect" && request.method === "POST") {
        return handleShopifyDisconnect(request, env);
      }
...
function isLiteralIpHost(hostname) {
  ...
  return normalized.includes(":") || isLiteralIpv4Address(normalized);
}
...
function isDisallowedWooCommerceHostname(hostname) {
  ...
  return isLiteralIpHost(normalized);
}
...
function assertSafeWooCommerceStoreUrlForFetch(storeUrl) {
...
function getWiseEnvironmentMode(env) {
  const explicit = String(env?.WISE_ENVIRONMENT || "").trim().toLowerCase();
  if (explicit === "sandbox" || explicit === "test") return "sandbox";
  if (explicit === "production" || explicit === "prod" || explicit === "live") return "production";
  const apiBase = String(env?.WISE_API_BASE_URL || "").trim().toLowerCase();
  if (apiBase.includes("sandbox")) return "sandbox";
  return "production";
}

function getWiseApiBaseUrl(env) {
  const explicit = String(env?.WISE_API_BASE_URL || "").trim();
  if (explicit) return explicit.replace(/\/+$/, "");
  return getWiseEnvironmentMode(env) === "sandbox"
    ? WISE_SANDBOX_API_BASE_URL
    : WISE_DEFAULT_API_BASE_URL;
}

function getWiseWebhookPublicKeyPem(env) {
  const explicit = String(env?.WISE_WEBHOOK_PUBLIC_KEY || "").trim();
...
function getWiseApiToken(env) {
  return String(env?.WISE_API_TOKEN || "").trim();
}

function getWiseProfileId(env) {
  return String(env?.WISE_PROFILE_ID || "").trim();
}

function getWiseBalanceId(env) {
  return String(env?.WISE_BALANCE_ID || "").trim();
}

function getWiseBalanceCurrency(env) {
  return String(env?.WISE_BALANCE_CURRENCY || DEFAULT_BILLING_CURRENCY).trim()
    || DEFAULT_BILLING_CURRENCY;
}

function getWiseStatementLookbackDays(env, defaultValue = WISE_DEFAULT_LOOKBACK_DAYS) {
  const numeric = Number(env?.WISE_STATEMENT_LOOKBACK_DAYS);
  if (!Number.isFinite(numeric)) return defaultValue;
  return Math.max(1, Math.min(469, Math.round(numeric)));
}

function getWiseConfigSummary(env) {
  return {
    configured: Boolean(getWiseApiToken(env) && getWiseProfileId(env) && getWiseBalanceId(env)),
    environment: getWiseEnvironmentMode(env),
    api_base_url: getWiseApiBaseUrl(env),
    profile_id: getWiseProfileId(env) || null,
    balance_id: getWiseBalanceId(env) || null,
    currency: getWiseBalanceCurrency(env),
  };
}

function assertWiseStatementConfig(env) {
  if (!getWiseApiToken(env)) {
    throw new Error("WISE_API_TOKEN is required for Wise reconciliation.");
  }
  if (!getWiseProfileId(env) || !getWiseBalanceId(env)) {
...
function pemToArrayBuffer(pem) {
  const normalized = String(pem || "").replace(/-----[^-]+-----/g, "").replace(/\s+/g, "");
  const bytes = Uint8Array.from(atob(normalized), (char) => char.charCodeAt(0));
  return bytes.buffer;
}

async function verifyWiseWebhookSignature(env, rawBody, receivedSignature) {
  const signature = String(receivedSignature || "").trim();
  const publicKey = getWiseWebhookPublicKeyPem(env);
...
function getLinkedInApiVersion(env) {
  return String(env.LINKEDIN_API_VERSION || DEFAULT_LINKEDIN_API_VERSION).trim() || DEFAULT_LINKEDIN_API_VERSION;
}

function getLinkedInRedirectUri(request, env) {
  const explicit = String(env.LINKEDIN_REDIRECT_URI || "").trim();
  if (explicit) return explicit;
  return `${getPublicOrigin(request)}/api/linkedin/callback`;
}

function getLinkedInScopes(env) {
  const scopes = String(env.LINKEDIN_SCOPES || "rw_organization_admin w_organization_social")
    .split(/\s+/)
    .map((value) => String(value || "").trim())
    .filter(Boolean);
  return scopes.length ? scopes : ["rw_organization_admin", "w_organization_social"];
}

function assertLinkedInConfig(env) {
  if (!String(env.LINKEDIN_CLIENT_ID || "").trim()) {
    throw new Error("LINKEDIN_CLIENT_ID is required for LinkedIn posting.");
  }
  if (!String(env.LINKEDIN_CLIENT_SECRET || "").trim()) {
...
async function buildInvoiceViewUrl(env, invoice, reminderStage, options = {}) {
  const invoiceId = String(invoice?.id || "").trim();
  if (!invoiceId) return "#";
  const token = await makeSignedPayloadToken(
    env,
    {
      type: "invoice_view",
      invoiceId,
  ...
  const publicAppUrl =
    ...
    || getPublicAppUrl(env, options?.request || null);
...
function encodeBasicAuth(username, password) {
  return btoa(`${String(username || "")}:${String(password || "")}`);
}
...
async function verifyShopifyWebhookBodyHmac(env, rawBody, receivedHmac) {
  const incoming = String(receivedHmac || "").trim();
  if (!incoming || !env.SHOPIFY_API_SECRET) return false;
  const digestBytes = await hmacSha256Bytes(env.SHOPIFY_API_SECRET, String(rawBody || ""));
  return timingSafeEqual(incoming, bytesToBase64(digestBytes));
}

async function verifyShopifySessionToken(env, token) {
  if (!env.SHOPIFY_API_SECRET || !env.SHOPIFY_API_KEY) return null;
  ...
  let payload = null;
  try {
    header = JSON.parse(new TextDecoder().decode(fromBase64Url(encodedHeader)));
  ...
  if (
    ...
    || !timingSafeEqual(toBase64Url(receivedSignature), toBase64Url(expectedSignature))
  ) {
    return null;
  }

  const audience = Array.isArray(payload.aud) ? payload.aud : [payload.aud];
  const audienceValid = audience.some(
    (value) => String(value || "").trim() === String(env.SHOPIFY_API_KEY || "").trim()
  );
  if (!audienceValid) {
    return null;
  }
...
function hasInviteAdminEmailAllowlistAccess(user, env) {
...
function canManageRegistrationInvites(user, env) {
  ...
  // Only trust server-managed app metadata or the explicit email allowlist.
  return hasServerManagedInviteAdminAccess(user) || hasInviteAdminEmailAllowlistAccess(user, env);
}
...
async function handleInvoicePdfView(request, env, url) {
  ...
  try {
    ...
    const variant = await loadStoredBillingInvoicePdfVariant(env, invoice, reminderStage, {
      allowFallback: false,
    }).catch(() => null);
    if (!variant?.bytes) {
      return new Response("Invoice not found.", {
...
async function getClientBillingPreferenceForUser(env, userId) {
  ...
  const trustedRows = await filterTrustedClientBillingPreferenceRows(env, rows);
...
function getBillingIbanConfig(env) {
  return {
    ...
    address: String(env.BILLING_IBAN_ADDRESS || DEFAULT_IBAN_ADDRESS).trim(),
...
async function updateBillingTopupFields(env, topupId, patch = {}) {
...
function buildWiseReceiptExternalKey(prefix, payload = {}) {
  ...
  return `wise:${hashString64(rawKey)}`;
}

function extractWiseReceiptFromStatementTransaction(env, transaction = {}) {
...
function extractWiseReceiptCandidatesFromWebhookPayload(env, payload = {}) {
...
async function getBillingBankReceiptById(env, receiptId, { allowMissing = false } = {}) {
...
async function getBillingBankReceiptByExternalKey(env, externalKey, { allowMissing = false } = {}) {
...
async function listBillingBankReceipts(env, { limit = 80, statuses = [], allowMissing = false } = {}) {
...
async function upsertBillingBankReceipt(env, candidate = {}) {
  const existing = await getBillingBankReceiptByExternalKey(env, candidate?.external_key, {
    allowMissing: true,
  });
...
async function updateBillingBankReceiptRow(env, receipt, patch = {}) {
  const current = receipt?.id
    ? receipt
    : await getBillingBankReceiptById(env, receipt, { allowMissing: false });
  if (!current?.id) {
    throw new Error("Bank receipt not found.");
  }
  return upsertBillingBankReceipt(env, {
    ...current,
    ...patch,
...
async function getWiseWebhookEventByDeliveryId(env, deliveryId, { allowMissing = false } = {}) {
...
async function upsertWiseWebhookEvent(env, payload = {}) {
  const existing = await getWiseWebhookEventByDeliveryId(env, payload?.delivery_id, { allowMissing: true });
  const record = {
...
async function updateWiseWebhookEventStatus(env, deliveryId, status, processingError = "") {
  const existing = await getWiseWebhookEventByDeliveryId(env, deliveryId, { allowMissing: false });
  if (!existing?.delivery_id) {
    throw new Error("Wise webhook delivery not found.");
  }
  return upsertWiseWebhookEvent(env, {
    ...existing,
...
async function getBillingTopupByReference(env, reference, { allowMissing = false } = {}) {
...
async function getBillingTopupById(env, topupId, { allowMissing = false } = {}) {
  const safeTopupId = String(topupId || "").trim();
...
async function upsertBillingTopupInvoice(env, topup, user = null) {
  if (!topup?.id) return null;
  const existing = await getBillingInvoiceBySourceTopupId(env, topup.id, {
    withItems: false,
    allowMissing: true,
  });
  ...
  await replaceBillingInvoiceItems(env, invoice.id, buildTopupInvoiceItems(topup));
  return getBillingInvoiceById(env, invoice.id, { withItems: true });
}

async function linkBillingTopupInvoice(env, topup, invoice) {
...
async function ensureBillingTopupInvoiceAndSend(env, topupId, options = {}) {
  ...
  if (alreadySent) {
    let finalInvoice =
      ...
      || invoice;
    if (shouldEnsurePdf) {
      await getApprovedBillingInvoicePdfExport(env, finalInvoice.id, { reminderStage: 0 });
      finalInvoice =
        await getBillingInvoiceById(env, invoice.id, { withItems: true }).catch(() => finalInvoice)
        || finalInvoice;
    }
    const finalTopup = await linkBillingTopupInvoice(env, linkedTopup || topup, finalInvoice).catch(
      () => linkedTopup || topup
  ...
  const readyInvoice =
    await getBillingInvoiceById(env, invoice.id, { withItems: true }).catch(() => finalInvoice)
    || finalInvoice;
  const finalTopup = await linkBillingTopupInvoice(env, linkedTopup || topup, readyInvoice).catch(
    () => linkedTopup || topup
  );
  return {
    skipped: false,
...
async function findBillingInvoiceByReference(env, reference, { allowMissing = false } = {}) {
...
async function applyBillingBankReceiptResolution(
  ...
  const response = await supabaseServiceRequest(env, `/rest/v1/rpc/${WISE_APPLY_RECEIPT_RPC}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function resolveBillingBankReceiptTarget(env, receipt, resolution, target = "") {
  ...
  if (isUuid(resolvedTarget)) {
    return action === "topup"
      ? getBillingTopupById(env, resolvedTarget, { allowMissing: true })
      : getBillingInvoiceById(env, resolvedTarget, { withItems: false });
  }
  if (action === "topup") {
    const topupReference = normalizeTopupReferenceCandidate(resolvedTarget);
    return topupReference ? getBillingTopupByReference(env, topupReference, { allowMissing: true }) : null;
  }
  const invoiceReference = normalizeInvoiceReferenceCandidate(resolvedTarget);
  return invoiceReference ? findBillingInvoiceByReference(env, invoiceReference, { allowMissing: true }) : null;
}

async function tryAutoResolveBankReceipt(env, receipt, actor = "wise-auto") {
  const currentReceipt = receipt?.id
    ? await getBillingBankReceiptById(env, receipt.id, { allowMissing: false })
    : null;
  ...
  for (const candidate of referenceCandidates) {
    ...
    if (topupReference) {
      const topup = await getBillingTopupByReference(env, topupReference, { allowMissing: true });
    ...
    if (invoiceReference) {
      const invoice = await findBillingInvoiceByReference(env, invoiceReference, { allowMissing: true });
  ...
  if (topupMatches.length === 1 && invoiceMatches.length === 0) {
    await updateBillingBankReceiptRow(env, currentReceipt, {
      status: "pending",
      match_reason: "topup_reference_exact",
      metadata: metadataPatch,
    });
    const resolved = await applyBillingBankReceiptResolution(env, {
  ...
  if (invoiceMatches.length === 1 && topupMatches.length === 0) {
    ...
    if (outstandingCents <= 0) {
      const updated = await updateBillingBankReceiptRow(env, currentReceipt, {
...
async function fetchWiseBalanceStatement(env, { lookbackDays = WISE_DEFAULT_LOOKBACK_DAYS } = {}) {
  assertWiseStatementConfig(env);
  ...
  const url = new URL(
    `/v1/profiles/${encodeURIComponent(getWiseProfileId(env))}/balance-statements/${encodeURIComponent(
      getWiseBalanceId(env)
    )}/statement.json`,
    `${getWiseApiBaseUrl(env)}/`
  );
  url.searchParams.set("currency", getWiseBalanceCurrency(env));
  ...
  const response = await fetch(url.toString(), {
    method: "GET",
    headers: {
      Authorization: `Bearer ${getWiseApiToken(env)}`,
      "Content-Type": "application/json",
    },
  });
  if (!response.ok) {
...
async function syncWiseBalanceStatement(
  env,
  {
    lookbackDays = getWiseStatementLookbackDays(env),
    actor = "wise-sync",
  } = {}
) {
  const statement = await fetchWiseBalanceStatement(env, { lookbackDays });
  ...
  for (const transaction of transactions) {
    const candidate = extractWiseReceiptFromStatementTransaction(env, transaction);
    if (!candidate) {
      summary.ignored += 1;
      continue;
    }
    ...
    const receipt = await upsertBillingBankReceipt(env, candidate);
    if (existing?.id) {
      summary.updated += 1;
    } else {
      summary.created += 1;
    }
    const result = await tryAutoResolveBankReceipt(env, receipt, actor);
...
async function acceptWiseWebhookDelivery(env, rawBody, headers = {}) {
  const deliveryId = String(headers?.deliveryId || "").trim()
    || `wise-${hashString64(String(rawBody || ""))}`;
  let payload = null;
  try {
  ...
  const signatureValid = await verifyWiseWebhookSignature(
    env,
    textEncoder.encode(String(rawBody || "")),
    headers?.signature
  );
  if (!signatureValid) {
  ...
  const eventRecord = await upsertWiseWebhookEvent(env, {
...
async function processWiseWebhookAcceptedEvent(env, accepted) {
  ...
  try {
    if (accepted?.isTest) {
      await updateWiseWebhookEventStatus(env, deliveryId, "ignored", "");
      return {
        scanned: 0,
        created: 0,
        updated: 0,
    ...
    let summary = null;
    try {
      if (getWiseConfigSummary(env).configured) {
        summary = await syncWiseBalanceStatement(env, {
    ...
    if (!summary || Number(summary.scanned || 0) === 0) {
      ...
      const candidates = extractWiseReceiptCandidatesFromWebhookPayload(env, accepted?.payload || {});
      for (const candidate of candidates) {
        fallbackSummary.scanned += 1;
        const existing = await getBillingBankReceiptByExternalKey(env, candidate.external_key, {
          allowMissing: true,
        });
        const receipt = await upsertBillingBankReceipt(env, {
          ...candidate,
          delivery_id: deliveryId,
        });
        if (existing?.id) {
        ...
        const result = await tryAutoResolveBankReceipt(env, receipt, "wise-webhook");
...
async function listBillingInvoicesForPeriod(
  ...
  try {
    ...
    grouped.forEach((group, key) => {
      const sorted = group.slice().sort(compareCanonicalBillingInvoices);
      if (sorted.length > 1) {
        console.error("[billing invoice duplicate period]", key, sorted.map((invoice) => ({
          id: invoice?.id || null,
...
async function createBillingTopupInvoice(env, invoicePayload) {
  const response = await supabaseServiceRequest(env, `/rest/v1/${BILLING_INVOICES_TABLE}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Prefer: "return=representation",
  ...
  if (!response.ok) {
    ...
    if ((details.includes("23505") || /duplicate key/i.test(details)) && invoicePayload?.source_topup_id) {
      const existing = await getBillingInvoiceBySourceTopupId(env, invoicePayload.source_topup_id, {
        withItems: false,
        allowMissing: true,
      });
...
async function renderSelectableInvoicePdfBatch(env, payloads = [], options = {}) {
  ...
  if (jobs.length > MAX_BILLING_INVOICE_RENDER_BATCH_VARIANTS) {
    const error = new Error(
...
async function isTrustedClientBillingPreferenceUpdater(env, userId) {
  const safeUserId = String(userId || "").trim();
  if (!safeUserId) return false;
  const user = await getSupabaseUserById(env, safeUserId);
  return canManageRegistrationInvites(user, env);
}

async function filterTrustedClientBillingPreferenceRows(env, rows = []) {
  ...
  for (const row of Array.isArray(rows) ? rows : []) {
    ...
    if (trustedUpdater == null) {
      try {
        trustedUpdater = await isTrustedClientBillingPreferenceUpdater(env, updaterId);
      } catch (_error) {
...
async function listClientBillingPreferences(env, limit = 2000) {
  ...
  const rows = await response.json().catch(() => []);
  return filterTrustedClientBillingPreferenceRows(env, rows);
}
    ...
    (async () => {
      ...
      for (const invoiceIdChunk of chunkArray(nonTopupInvoiceIds, 100)) {
        ...
        const rows = await fetchAllSupabaseRows(
          ...
          {
            select: "invoice_id,service_type,quantity,amount_ex_vat",
            invoice_id: inFilter,
          },
          { pageSize: 500, allowMissingSchema: true }
        ).catch(() => []);
...
async function fetchAllSupabaseRows(env, tableName, paramsInit = {}, options = {}) {
...
async function deleteSupabaseRows(env, tableName, paramsInit = {}, options = {}) {
  const params = new URLSearchParams();
  Object.entries(paramsInit || {}).forEach(([key, value]) => {
    if (value === undefined || value === null || value === '') return;
    params.set(key, String(value));
...
async function patchSupabaseRows(env, tableName, paramsInit = {}, patch = {}, options = {}) {
  const params = new URLSearchParams();
  Object.entries(paramsInit || {}).forEach(([key, value]) => {
    if (value === undefined || value === null || value === '') return;
    params.set(key, String(value));
...
async function insertPrivacyRequest(env, record, options = {}) {
  const nowIso = new Date().toISOString();
  const response = await supabaseServiceRequest(env, `/rest/v1/${PRIVACY_REQUESTS_TABLE}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Prefer: 'return=representation',
...
async function updatePrivacyRequest(env, requestId, patch = {}, options = {}) {
  const safeRequestId = String(requestId || '').trim();
  if (!safeRequestId) return null;
  const response = await supabaseServiceRequest(
    env,
...
async function getSingleOptionalSupabaseRow(env, tableName, paramsInit = {}) {
  const rows = await fetchAllSupabaseRows(env, tableName, paramsInit, {
    pageSize: 1,
...
async function listProviderConnectionsForPrivacyExport(env, userId) {
...
async function listHistoryRowsForPrivacyExport(env, userId) {
  return fetchAllSupabaseRows(
    env,
    HISTORY_TABLE,
    {
...
async function listBillingInvoicesForPrivacyExport(env, userId) {
  return fetchAllSupabaseRows(
    env,
    BILLING_INVOICES_TABLE,
    {
...
async function listBillingInvoiceItemsForPrivacyExport(env, invoiceIds = []) {
  const items = [];
...
async function listBillingTopupsForPrivacyExport(env, userId) {
  return fetchAllSupabaseRows(
    env,
...
async function listBillingWalletTransactionsForPrivacyExport(env, userId) {
  return fetchAllSupabaseRows(
    env,
    BILLING_WALLET_TRANSACTIONS_TABLE,
    {
...
async function listClickwrapAcceptancesForPrivacyExport(env, userId) {
  return fetchAllSupabaseRows(
    env,
...
async function listPrivacyRequestsForPrivacyExport(env, userId) {
  return fetchAllSupabaseRows(
    env,
...
async function buildPrivacyExportBundle(env, user) {
  const userId = String(user?.id || '').trim();
  if (!userId) {
    throw new Error('Authentication required.');
  }
  const [
    ...
    getSingleOptionalSupabaseRow(env, 'account_settings', {
      select: '*',
      user_id: `eq.${userId}`,
    }),
    listProviderConnectionsForPrivacyExport(env, userId),
    listHistoryRowsForPrivacyExport(env, userId),
    listBillingInvoicesForPrivacyExport(env, userId),
    listBillingTopupsForPrivacyExport(env, userId),
    listBillingWalletTransactionsForPrivacyExport(env, userId),
    listClickwrapAcceptancesForPrivacyExport(env, userId),
    listPrivacyRequestsForPrivacyExport(env, userId),
    getSingleOptionalSupabaseRow(env, BILLING_WALLET_TABLE, {
...
async function handlePrivacyExport(request, env) {
  ...
  try {
    const { bundle, summary } = await buildPrivacyExportBundle(env, user);
    if (requestRecord?.id) {
    ...
    return jsonResponse({ error: error?.message || 'Could not build privacy export.' }, 500);
  }
}
...
async function handlePrivacyDelete(request, env) {
  ...
  if (!requestRecord?.id) {
    return jsonResponse({ error: 'Privacy request storage is unavailable.' }, 503);
  }
  return jsonResponse(
    {
      ok: true,
...
async function deleteShopifyConnection(env, userId, shop) {
  const normalizedShop = normalizeShopDomain(shop);
  if (!userId || !normalizedShop) return;
  await deleteSupabaseRows(env, 'provider_connections', {
    user_id: `eq.${userId}`,
    provider: 'eq.shopify',
...
async function deleteWixConnection(env, userId, instanceId) {
...
async function deleteWooCommerceConnection(env, userId, storeUrl) {
  const normalizedStoreUrl = normalizeWooCommerceStoreUrl(storeUrl);
...
async function deleteShopifyConnectionsByShop(env, shop) {
  const normalizedShop = normalizeShopDomain(shop);
...
async function deleteShopifySessionsByShop(env, shop) {
  const normalizedShop = normalizeShopDomain(shop);
  if (!normalizedShop) return;
  await deleteSupabaseRows(env, 'Session', {
...
async function listShopifyConnectionsByShop(env, shop) {
  const normalizedShop = normalizeShopDomain(shop);
  if (!normalizedShop) return [];
  return fetchAllSupabaseRows(
    env,
...
async function resolveShopifyPrivacyAccessToken(env, connections = []) {
...
async function fetchShopifyOrdersByIds(env, shop, accessToken, orderIds = []) {
  const normalizedShop = normalizeShopDomain(shop);
  ...
  for (const idChunk of chunkArray(safeOrderIds, 100)) {
    const url = new URL(`https://${normalizedShop}/admin/api/${apiVersion}/orders.json`);
...
async function processShopifyCustomerPrivacyOperation(env, topic, shop, payload = {}) {
  const normalizedShop = normalizeShopDomain(shop);
  const nowIso = new Date().toISOString();
  const connections = await listShopifyConnectionsByShop(env, normalizedShop).catch(() => []);
  ...
  const accessToken = await resolveShopifyPrivacyAccessToken(env, connections);
  const requestedOrderIds = normalizeShopifyComplianceOrderIds(payload);
  let fetchedOrders = [];
  if (requestedOrderIds.length && accessToken) {
    fetchedOrders = await fetchShopifyOrdersByIds(env, normalizedShop, accessToken, requestedOrderIds).catch(
      () => []
    );
  }
  ...
  const historyRows = await fetchAllSupabaseRows(env, HISTORY_TABLE, historyFilters, { pageSize: 500 });
...
async function processShopifyShopRedactOperation(env, shop) {
  const normalizedShop = normalizeShopDomain(shop);
  const nowIso = new Date().toISOString();
  const connections = await listShopifyConnectionsByShop(env, normalizedShop).catch(() => []);
  ...
  const deletedConnections = await deleteShopifyConnectionsByShop(env, shop).catch(() => []);
  await deleteShopifySessionsByShop(env, shop).catch(() => {});
...
async function processShopifyComplianceRequest(env, topic, shop, payload = {}) {
  const customer =
  ...
  if (topic === 'shop/redact') {
    resultPayload = await processShopifyShopRedactOperation(env, shop);
  } else {
    resultPayload = await processShopifyCustomerPrivacyOperation(env, topic, shop, payload);
  }
...
async function runPrivacyMaintenance(env, options = {}) {
  const now = new Date();
...
async function handleAdminPrivacyMaintenanceRun(request, env) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: 'Authentication required.' }, 401);
  }
  if (!canManageRegistrationInvites(user, env)) {
    return jsonResponse({ error: 'You are not allowed to run privacy maintenance.' }, 403);
  }
  try {
    const result = await runPrivacyMaintenance(env, {
      triggeredBy: user.id,
    });
    return jsonResponse({
      ok: true,
    ...
    return jsonResponse({ error: error?.message || 'Privacy maintenance failed.' }, 500);
  }
}
...
async function getShopifyConnectionByShop(env, shop, options = {}) {
  const { includeSettings = false } = options;
  const normalizedShop = normalizeShopDomain(shop);
...
async function resolveShopifyEmbeddedSessionContext(env, token) {
  const session = await verifyShopifySessionToken(env, token);
  if (!session?.shop) {
    return null;
  }

  const connection = await getShopifyConnectionByShop(env, session.shop).catch(() => null);
  ...
  let portalUser = null;
  try {
    portalUser = await getSupabaseUserById(env, connection.user_id);
  } catch (_error) {
    portalUser = {
...
async function upsertWooCommerceConnection(env, userId, storeUrl, consumerKey, consumerSecret) {
  const nowIso = new Date().toISOString();
...
function getLinkedInApiHeaders(env, accessToken, extraHeaders = {}) {
  return {
    Authorization: `Bearer ${accessToken}`,
    "Linkedin-Version": getLinkedInApiVersion(env),
    "X-Restli-Protocol-Version": "2.0.0",
    ...extraHeaders,
  };
}

async function fetchLinkedInJson(env, accessToken, path, options = {}) {
  const response = await fetch(`https://api.linkedin.com${path}`, {
    ...options,
    headers: getLinkedInApiHeaders(env, accessToken, {
      "Content-Type": "application/json",
      ...(options.headers || {}),
    }),
  });
...
async function parseStoredLinkedInConnection(env, encryptedToken) {
  if (!String(encryptedToken || "").trim()) return null;
  const decrypted = await decryptToken(env, encryptedToken);
...
async function upsertLinkedInConnection(env, userId, payload = {}) {
  const nowIso = new Date().toISOString();
      ...
      body: JSON.stringify([
        {
          ...
          access_token: await encryptToken(env, JSON.stringify(normalizedPayload)),
          updated_at: nowIso,
...
async function getLinkedInConnection(env, userId) {
  const params = new URLSearchParams();
...
async function listLinkedInOrganizations(env, accessToken) {
...
async function refreshLinkedInConnectionOrganizations(env, row, stored) {
  const organizations = await listLinkedInOrganizations(env, stored.accessToken);
  ...
  await upsertLinkedInConnection(env, row.user_id, {
    ...nextPayload,
...
async function setWixConnectionStatus(env, userId, instanceId, status) {
  const params = new URLSearchParams();
  params.set("user_id", `eq.${userId}`);
  params.set("provider", "eq.wix");
  params.set("shop_domain", `eq.${instanceId}`);
...
async function setWooCommerceConnectionStatus(env, userId, storeUrl, status) {
  const params = new URLSearchParams();
  params.set("user_id", `eq.${userId}`);
  params.set("provider", "eq.woocommerce");
  params.set("shop_domain", `eq.${storeUrl}`);
...
async function parseWooCommerceCredentials(env, token) {
  let decrypted = "";
  try {
    decrypted = await decryptToken(env, token);
  } catch (error) {
...
async function fetchShopifyOrders(
  ...
  await Promise.all(
    resolvedFinancialStatuses.map(async (financialStatus) => {
      ...
      ordersUrl.searchParams.set("fulfillment_status", "any");
...
async function fetchWooCommerceOrders(
  ...
  const normalizedStatuses = normalizeWooCommerceImportStatuses(selectedStatuses);
  const safeStoreUrl = assertSafeWooCommerceStoreUrlForFetch(storeUrl);
  const ordersUrl = new URL(`${safeStoreUrl.replace(/\/+$/, "")}/wp-json/wc/v3/orders`);
...
async function fetchWooCommerceGeneralSettings(storeUrl, consumerKey, consumerSecret) {
  const safeStoreUrl = assertSafeWooCommerceStoreUrlForFetch(storeUrl);
  const settingsUrl = new URL(`${safeStoreUrl.replace(/\/+$/, "")}/wp-json/wc/v3/settings/general`);
...
async function sendBillingInvoiceById(env, invoiceId, options = {}) {
  ...
  const viewUrl = await buildInvoiceViewUrl(env, invoiceWithItems, desiredStage, {
    publicAppUrl: options?.publicAppUrl,
  });
...
async function handleAdminInvoiceAccountingExport(request, env, ctx) {
  ...
  if (!canManageRegistrationInvites(user, env)) {
    return jsonResponse({ error: "You are not allowed to export invoices for accounting." }, 403);
  }
  let body = {};
  try {
    body = await readJsonBody(request);
  ...
  if (!invoiceIds.length) {
    return jsonResponse({ error: "Select at least one invoice to export." }, 400);
  }
  ...
  if (Number.isNaN(exportedAtDate.getTime())) {
    return jsonResponse({ error: "Export date is invalid." }, 400);
  }
...
async function handleBillingInvoiceDetail(request, env, url) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: "Authentication required." }, 401);
  }
...
async function handleAdminInvoicesRun(request, env) {
  ...
  try {
    const publicAppUrl = getPublicAppUrl(env, request);
    const result = await buildInvoicesFromLabelHistory(env, {
      ...runRequest,
      publicAppUrl,
    });
    let reminders = [];
    if (runRequest.mode !== "preview" && runRequest.includeReminders) {
      reminders = await runInvoiceReminders(env, { limit: 300, publicAppUrl });
    }
    return jsonResponse({
      ok: true,
      run: result,
      ...
      return {
        ...
        payload: {
          invoice: {
            ...emailInvoice,
            items,
            reference: getBillingInvoiceReference(emailInvoice),
          },
      ...
      ? await renderSelectableInvoicePdf(
          env,
          {
            invoice: {
              ...testInvoice,
              items: testItems,
              reference: getBillingInvoiceReference(testInvoice),
            },
            reminderStage: 0,
...
async function handleAdminTopupInvoiceSendTest(request, env) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: "Authentication required." }, 401);
  }
  ...
  try {
    ...
    const renderedVariant = !providedPdfBase64 && env?.BROWSER
      ? await renderSelectableInvoicePdf(
          ...
          throw new Error(error?.message || "Could not render the approved top-up invoice PDF for the test email.");
        })
      : null;
    const pdfBytes = providedPdfBase64
      ? Uint8Array.from(atob(providedPdfBase64), (char) => char.charCodeAt(0))
      : renderedVariant?.bytes || null;
    if (!pdfBytes) {
      throw new Error("Approved top-up invoice PDF is unavailable for the test email.");
    }
    const resendResponse = await sendResendEmail(env, {
      to: toEmail,
      subject: buildInvoiceEmailSubject(testInvoice, { isReminder: false, reminderStage: 0 }),
    ...
    return jsonResponse({ error: error?.message || "Could not send test top-up invoice email." }, 500);
  }
}
...
async function handleWiseWebhook(request, env, ctx) {
  let rawBody = "";
  try {
  ...
  try {
    const accepted = await acceptWiseWebhookDelivery(env, rawBody, {
      signature: request.headers.get("x-signature-sha256"),
      deliveryId: request.headers.get("x-delivery-id"),
      subscriptionId: request.headers.get("x-subscription-id"),
      schemaVersion: request.headers.get("x-schema-version"),
      isTest: String(request.headers.get("x-test-notification") || "").trim().toLowerCase() === "true",
    });
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(
        processWiseWebhookAcceptedEvent(env, accepted).catch((error) => {
          console.error("[wise webhook]", error);
        })
      );
    } else {
      void processWiseWebhookAcceptedEvent(env, accepted).catch((error) => {
        console.error("[wise webhook]", error);
      });
    }
    return jsonResponse(
    ...
    return jsonResponse(
      { error: error?.message || "Could not accept Wise webhook." },
      Number(error?.statusCode) || 500
    );
  }
}

async function handleAdminWiseReceiptsList(request, env, url) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: "Authentication required." }, 401);
  }
  if (!canManageRegistrationInvites(user, env)) {
    return jsonResponse({ error: "You are not allowed to access Wise reconciliation." }, 403);
  }
  const limit = Math.max(1, Math.min(200, Number(url.searchParams.get("limit")) || 80));
  const statuses = String(url.searchParams.get("status") || "manual_review,pending")
    .split(",")
    .map((entry) => String(entry || "").trim().toLowerCase())
    .filter(Boolean);
  try {
    ...
    return jsonResponse(
      { error: error?.message || "Could not load Wise reconciliation queue." },
      500
    );
  }
}

async function handleAdminWiseSync(request, env) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: "Authentication required." }, 401);
  }
  if (!canManageRegistrationInvites(user, env)) {
    return jsonResponse({ error: "You are not allowed to sync Wise reconciliation." }, 403);
  }
  let body = {};
  try {
    body = await readJsonBody(request);
  ...
  try {
    const summary = await syncWiseBalanceStatement(env, {
    ...
    return jsonResponse({ error: error?.message || "Could not sync Wise statement." }, 500);
  }
}

async function handleAdminWiseReceiptResolve(request, env) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: "Authentication required." }, 401);
  }
  if (!canManageRegistrationInvites(user, env)) {
    return jsonResponse({ error: "You are not allowed to update Wise reconciliation." }, 403);
  }
  let body = {};
  try {
    body = await readJsonBody(request);
  ...
  const note = String(body?.note || "").trim();
  if (!receiptId) {
    return jsonResponse({ error: "Bank receipt id is required." }, 400);
  }
  if (!["topup", "invoice", "ignore"].includes(resolution)) {
    return jsonResponse({ error: "Resolution must be topup, invoice, or ignore." }, 400);
  }

  try {
    const receipt = await getBillingBankReceiptById(env, receiptId, { allowMissing: false });
    if (!receipt?.id) {
      return jsonResponse({ error: "Bank receipt not found." }, 404);
    }
    if (resolution === "ignore") {
      const updated = await applyBillingBankReceiptResolution(env, {
        receiptId,
        resolution,
        actor: normalizeEmail(user.email || "") || user.id,
        note,
    ...
    const targetRow = await resolveBillingBankReceiptTarget(env, receipt, resolution, target);
    if (!targetRow?.id) {
      return jsonResponse(
        {
          error:
    ...
    if (resolution === "invoice") {
      ...
      if (outstandingCents <= 0) {
        return jsonResponse({ error: "This invoice is already fully paid." }, 400);
      }
    ...
    if (resolution === "topup" && targetRow?.id) {
      try {
        await ensureBillingTopupInvoiceAndSend(env, targetRow.id, {
          publicAppUrl: getPublicAppUrl(env, request),
        });
    ...
    return jsonResponse({ error: error?.message || "Could not resolve bank receipt." }, 500);
  }
}
...
async function handleBillingTopupInvoicePdf(request, env, url, ctx) {
  ...
  try {
    ...
    let { invoice } = await resolveCanonicalBillingTopupInvoice(env, topup, {
      user,
      withItems: false,
    });
    if (!invoice?.id) {
    ...
    try {
      const ensuredPdf = await getApprovedBillingInvoicePdfExport(env, invoice.id, { reminderStage: 0 });
      if (ensuredPdf?.bytes) {
        return new Response(ensuredPdf.bytes, {
          status: 200,
          headers: {
            "Content-Type": "application/pdf",
            "Content-Disposition": `attachment; filename="${String(ensuredPdf.filename || "invoice-shipide.pdf").replace(/"/g, "")}"`,
            "Cache-Control": "no-store",
          },
        });
      }
      invoice =
        await getBillingInvoiceById(env, invoice.id, { withItems: false }).catch(() => invoice)
        || invoice;
    } catch (_error) {
      // Fall back to the queued worker path below if a direct export cannot be resolved promptly.
    }
    let job = await enqueueInvoicePdfDocumentJob(env, invoice, {
      invoiceId: invoice.id,
      reminderStage: 0,
...
async function handleBillingTopupInvoiceRepair(request, env) {
  const user = await getAuthenticatedUser(request, env);
  if (!user?.id) {
    return jsonResponse({ error: "Authentication required." }, 401);
  }
  ...
  try {
    ...
    return jsonResponse({
      ...
      topup: result?.topup ? mapBillingTopupRow(result.topup) : mapBillingTopupRow(topup),
      invoice:
        result?.invoice
          ? buildInvoiceListResponseRows([result.invoice])[0] || null
          : null,
    });
  } catch (error) {
    return jsonResponse({ error: error?.message || "Could not repair top-up invoice." }, 500);
  }
}
...
async function handleBillingInvoicePdfRenderBatch(request, env) {
  ...
  if (rows.length > MAX_BILLING_INVOICE_RENDER_BATCH_VARIANTS) {
    return jsonResponse(
      {
        error: `Too many invoice variants requested. Maximum is ${MAX_BILLING_INVOICE_RENDER_BATCH_VARIANTS}.`,
      },
      400
    );
  }
...
async function handleShopifyPublicConfig(_request, env) {
  return jsonResponse({
    shopifyApiKey: String(env.SHOPIFY_API_KEY || "").trim(),
    appBridgeCdnUrl: "https://cdn.shopify.com/shopifycloud/app-bridge.js",
    embedded: true,
  });
}

async function handleShopifyEmbeddedSession(request, env, requestUrl) {
  try {
    assertConfig(env);
    ...
    const context = await resolveShopifyEmbeddedSessionContext(env, sessionToken);
    ...
    return jsonResponse({
      ...
      portalUser: context.portalUser
        ...
        : null,
      portalUrl: `${getPublicAppUrl(env, request)}/label-info`,
    });
  } catch (error) {
    return jsonResponse(
      { error: error?.message || "Could not verify Shopify embedded session." },
      500
    );
  }
}
...
async function handleShopifyComplianceWebhook(request, env) {
  ...
  const receivedHmac = String(request.headers.get('x-shopify-hmac-sha256') || '').trim();

  if (!SHOPIFY_COMPLIANCE_WEBHOOK_TOPICS.includes(topic)) {
    return jsonResponse({ error: 'Unsupported Shopify compliance webhook topic.' }, 400);
  }
  if (!(await verifyShopifyWebhookBodyHmac(env, rawBody, receivedHmac))) {
  ...
  try {
    const result = await processShopifyComplianceRequest(env, topic, shop, payload);
    return jsonResponse({
      ok: true,
      topic,
      shop,
...
async function handleWixSettingsGet(request, env, requestUrl) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    const connection = await getWixConnection(env, user.id, requestedInstanceId);
    if (!connection) {
    ...
    return jsonResponse({ error: error?.message || "Failed to load Wix settings." }, 500);
  }
}
...
async function handleWixLocations(request, env, requestUrl) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    const locations = (await fetchWixLocations(accessToken))
      ...
      .filter((location) => location.id && location.name);
    return jsonResponse({ instanceId: connection.shop_domain, locations });
  } catch (error) {
    return jsonResponse({ error: error?.message || "Failed to load Wix business locations." }, 500);
  }
}

async function handleWixSettingsPost(request, env) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    if (!selectedStatuses.length) {
      return jsonResponse({ error: "Select at least one Wix fulfillment status to import." }, 400);
    }
    ...
    const refreshedConnection = await getWixConnection(env, user.id, connection.shop_domain, {
      includeSettings: true,
    });
    ...
    return jsonResponse({ error: error?.message || "Failed to save Wix settings." }, 500);
  }
}

async function handleWixDisconnect(request, env) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    if (!connection) {
      return jsonResponse({ disconnected: false, connection: null });
    }
    await deleteWixConnection(env, user.id, connection.shop_domain);
    return jsonResponse({
      disconnected: true,
      connection: {
        instanceId: connection.shop_domain,
...
async function handleWooCommerceCallback(request, env, callbackUrl) {
  try {
    ...
    storeUrl = assertSafeWooCommerceStoreUrlForFetch(storeUrl);
...
async function handleWooCommerceConnection(request, env) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    }
    const row = await getWooCommerceConnection(env, user.id, "");
    if (!row) {
      return jsonResponse({ connected: false, connection: null });
    }
    return jsonResponse({
    ...
    return jsonResponse(
      { error: error?.message || "Could not load WooCommerce connection." },
      500
    );
  }
}
...
async function handleWooCommerceDisconnect(request, env) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    await deleteWooCommerceConnection(env, user.id, connection.shop_domain);
    return jsonResponse({
      disconnected: true,
      connection: {
        storeUrl: connection.shop_domain,
    ...
    return jsonResponse({ error: error?.message || "Failed to disconnect WooCommerce." }, 500);
  }
}

function getLinkedInCallbackRedirect(request, params = {}) {
  const url = new URL("/post", getPublicOrigin(request));
  Object.entries(params).forEach(([key, value]) => {
    if (value === undefined || value === null || value === "") return;
    url.searchParams.set(key, String(value));
  });
...
async function waitForLinkedInImageAvailable(env, accessToken, imageUrn) {
...
async function uploadLinkedInImage(env, accessToken, ownerUrn, imageDataUrl) {
  ...
  await waitForLinkedInImageAvailable(env, accessToken, imageUrn);
  return imageUrn;
}

async function handleLinkedInInstallLink(request, env) {
  try {
    assertLinkedInConfig(env);
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    }
    ...
    installUrl.searchParams.set("client_id", String(env.LINKEDIN_CLIENT_ID || "").trim());
    installUrl.searchParams.set("redirect_uri", getLinkedInRedirectUri(request, env));
    installUrl.searchParams.set("state", state);
    installUrl.searchParams.set("scope", getLinkedInScopes(env).join(" "));
    return jsonResponse({
      url: installUrl.toString(),
    });
  } catch (error) {
    return jsonResponse({ error: error?.message || "Could not start LinkedIn connect flow." }, 500);
  }
}

async function handleLinkedInCallback(request, env, callbackUrl) {
  try {
    assertLinkedInConfig(env);
    const linkedInError = String(callbackUrl.searchParams.get("error") || "").trim();
    if (linkedInError) {
      return redirectResponse(
        getLinkedInCallbackRedirect(request, {
          provider: LINKEDIN_PROVIDER,
          linkedin: "error",
          message:
            String(callbackUrl.searchParams.get("error_description") || "").trim()
    ...
    tokenBody.set("client_id", String(env.LINKEDIN_CLIENT_ID || "").trim());
    tokenBody.set("client_secret", String(env.LINKEDIN_CLIENT_SECRET || "").trim());
    tokenBody.set("redirect_uri", getLinkedInRedirectUri(request, env));
    ...
    try {
      organizations = await listLinkedInOrganizations(env, tokenPayload.access_token);
    ...
    const refreshExpiresAt = Number(tokenPayload?.refresh_token_expires_in)
      ...
      : "";
    await upsertLinkedInConnection(env, statePayload.userId, {
      accessToken: tokenPayload.access_token,
      refreshToken: tokenPayload.refresh_token || "",
      expiresAt,
      refreshExpiresAt,
      scope: tokenPayload.scope || getLinkedInScopes(env).join(" "),
      organizations,
...
async function handleLinkedInConnection(request, env, requestUrl) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    const configured =
      Boolean(String(env.LINKEDIN_CLIENT_ID || "").trim())
      && Boolean(String(env.LINKEDIN_CLIENT_SECRET || "").trim());
    if (!configured) {
      return jsonResponse(buildLinkedInConnectionPayload(null, null, false));
    }
    const row = await getLinkedInConnection(env, user.id);
    if (!row) {
      return jsonResponse(buildLinkedInConnectionPayload(null, null, true));
    }
    let stored = await parseStoredLinkedInConnection(env, row.access_token);
    ...
    if (shouldRefresh && !expired) {
      stored = await refreshLinkedInConnectionOrganizations(env, row, stored);
    }
    return jsonResponse(buildLinkedInConnectionPayload(row, stored, true));
  } catch (error) {
    return jsonResponse({ error: error?.message || "Could not load LinkedIn connection." }, 500);
  }
}

async function handleLinkedInSelectOrganization(request, env) {
  try {
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
      return jsonResponse({ error: "Authentication required." }, 401);
    ...
    const row = await getLinkedInConnection(env, user.id);
    if (!row) {
      return jsonResponse({ error: "LinkedIn is not connected for this account." }, 404);
    }
    const stored = await parseStoredLinkedInConnection(env, row.access_token);
    if (!stored?.accessToken) {
      return jsonResponse({ error: "LinkedIn connection data is invalid. Please reconnect." }, 400);
    }
    const body = await readJsonBody(request);
    const organizationUrn = String(body?.organizationUrn || "").trim();
    if (!organizationUrn) {
      return jsonResponse({ error: "Select a LinkedIn company page." }, 400);
    }
    const match = stored.organizations.find((entry) => entry.urn === organizationUrn);
    if (!match) {
      return jsonResponse({ error: "The selected LinkedIn company page is not available." }, 400);
    }
    const nextPayload = normalizeLinkedInStoredPayload({
      ...stored,
      selectedOrganizationUrn: organizationUrn,
    });
    await upsertLinkedInConnection(env, user.id, {
      ...nextPayload,
      connectedAt: row.connected_at || "",
    });
    return jsonResponse(buildLinkedInConnectionPayload(row, nextPayload, true));
  } catch (error) {
    return jsonResponse({ error: error?.message || "Could not save the LinkedIn page selection." }, 500);
  }
}
...
async function handleLinkedInDisconnect(request, env) {
  try {
    ...
    return jsonResponse({ error: error?.message || "Could not disconnect LinkedIn." }, 500);
  }
}

async function handleLinkedInPost(request, env) {
  try {
    assertLinkedInConfig(env);
    const user = await getAuthenticatedUser(request, env);
    if (!user?.id) {
    ...
    const row = await getLinkedInConnection(env, user.id);
    if (!row) {
      return jsonResponse({ error: "Connect LinkedIn before publishing." }, 404);
    }
    const stored = await parseStoredLinkedInConnection(env, row.access_token);
    if (!stored?.accessToken) {
      return jsonResponse({ error: "LinkedIn connection data is invalid. Please reconnect." }, 400);
    ...
    if (Number.isFinite(expiresAtMs) && Date.now() >= expiresAtMs - 60 * 1000) {
      return jsonResponse({ error: "Your LinkedIn connection has expired. Please reconnect." }, 400);
    }
    const selectedOrganization = getSelectedLinkedInOrganization(stored);
    if (!selectedOrganization?.urn) {
      return jsonResponse({ error: "Select a LinkedIn company page before publishing." }, 400);
    }
    const body = await readJsonBody(request);
    ...
    if (!caption) {
      return jsonResponse({ error: "Add a LinkedIn caption before publishing." }, 400);
    }
    if (!imageDataUrl) {
      return jsonResponse({ error: "The LinkedIn image payload is missing." }, 400);
    }
    ...
    const postResponse = await fetch("https://api.linkedin.com/rest/posts", {
      method: "POST",
      headers: getLinkedInApiHeaders(env, stored.accessToken, {
        "Content-Type": "application/json",
      }),
      body: JSON.stringify({
    ...
    return jsonResponse({ error: error?.message || "Could not publish to LinkedIn." }, 500);
  }
}
...
async function handleWooCommerceImportOrders(request, env) {
  ...
  try {
    ...
    if (
      ...
      await setWooCommerceConnectionStatus(
        env,
        authUserId,
        connectionStoreUrl,
        "token_invalid"
...
async function handleSettingsPost(request, env) {
  try {
    ...
    if (!selectedFinancialStatuses.length) {
      return jsonResponse({ error: "Select at least one Shopify status to import." }, 400);
    }
    if (!selectedFulfillmentStatuses.length) {
      return jsonResponse({ error: "Select at least one Shopify fulfillment status to import." }, 400);
    }
...
async function handleShopifyDisconnect(request, env) {
  try {
    ...
    await deleteShopifyConnection(env, user.id, connection.shop_domain);
    return jsonResponse({
      disconnected: true,
      connection: {
        shop: connection.shop_domain,
    ...
    return jsonResponse({ error: error?.message || "Failed to disconnect Shopify." }, 500);
  }
}
